...
describe('loadCsv', () => {
  ...
  it('should return expected CSV data', () => {
    ...
    createInlineCsv('Pos,Name,Rating,Fed,R1,R2,R3,Pts,Bu,Club\n' +
    ...
    expect(csv.header).toEqual(['Pos', 'Name', 'Rating', 'Fed', 'R1', 'R2', 'R3', 'Pts', 'Bu', 'Club']);
    const data = csv.data;
    expect(data).toBeTruthy();
    expect(data.length).toEqual(3);
    expect(data[0]).toEqual(['1', 'GM GLEK Igor', '2473', 'GER', '+48B', '+24N', '+5B', '6.5', '30', '']);
    expect(data[1]).toEqual(['2', 'IM BENITAH Yohan', '2530', 'FRA', '+39N', '+30B', '+6N', '6', '31', 'Lyon Olympique Echecs']);
    expect(data[2]).toEqual(['3', 'IM OLIVIER Jean-Christophe', '2440', 'FRA', '+41B', '+8N', '+13B', '6', '29', 'Club d\'Echecs de l\'Agglomeration Chambérienne']);
  });

  it('should throw expected error if target element id was not found', () => {
    expect(() => loadCsv('another')).toThrow('Target element with id=\'another\' not found!');
  });
